...
{	
	...
	if($chk > 0)
	{		  
		$chk_cdate = mysqli_query($dbLink,"select id from tbl_status where id='".mysqli_real_escape_string($dbLink,$data->status_id)."' 
		and user_id='".mysqli_real_escape_string($dbLink,$data->uid)."'");
		$chk_cstatus_num = mysqli_num_rows($chk_cdate);
		if($chk_cstatus_num>0)
		{				
			$row_cdate = mysqli_query($dbLink,"select *,date_format(fdate,'%m/%d/%Y')as fdate,date_format(tdate,'%m/%d/%Y')as tdate from tbl_status where user_id='".mysqli_real_escape_string($dbLink,$data->uid)."' and id='".mysqli_real_escape_string($dbLink,$data->status_id)."'");
